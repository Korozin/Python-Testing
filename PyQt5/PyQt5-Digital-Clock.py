...
class ClockApp(QWidget):
    def __init__(self):
        ...
        for i in range(11):
            img = QImage.fromData(base64.b64decode(image_map[' ']))
            label = QLabel()
        ...
        # Create a horizontal layout and add the image labels to it
        layout = QHBoxLayout()
    ...
    def update_clock(self):
        ...
        for i, base64_data in enumerate(formatted_time):
            img = QImage.fromData(base64.b64decode(base64_data))
            self.image_labels[i].setPixmap(QPixmap.fromImage(img))

if __name__ == '__main__':
    app = QApplication(sys.argv)
    app.setStyleSheet("QLabel { color: white; font-size: 30px; }")
    clock = ClockApp()
    clock.setFixedSize(440, 70)
